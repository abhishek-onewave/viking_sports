...
export default function ValuationForm() {
  ...
  // Only the small metadata file loads up front. Comps are sharded by player
  // initial and fetched when the user names a player — the full corpus is
  // 15.7 MB, the shard they need is at most 342 KB gzipped.
  useEffect(() => {
    loadCompsMeta().then(setMeta).catch((e: Error) => setLoadError(e.message));
  }, []);

  useEffect(() => {
    if (player.trim().length < 2) return;
    loadShard(player).then(setLots).catch((e: Error) => setLoadError(e.message));
  }, [player]);
    ...
    return (
      <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-6 text-red-200">
        ...
        <p className="mt-2 text-sm text-red-200/70">
          ...
          <code className="mx-1">public/data/comps/</code>.
        </p>
      </div>
    );
  }
